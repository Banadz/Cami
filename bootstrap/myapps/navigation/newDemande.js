$(document).ready(function () {
		...
		$(".titihieKategory").each(function () {
			$(this).on("click", function () {
				...
				$(".zoneCategorie").each(function () {
					$(this).attr(
						"value",
						(benzema[0].trim() + " - " + benzema[1].trim()).trim()
					);
	...
	$("body").on("click", "#validDemande", function () {
		...
		let big_data = [];
		Donnetable.rows().every(function () {
			let rowData = this.data();
			// Exclure la cinquième colonne (indice 4) en supprimant cet élément du tableau rowData
			rowData.splice(6, 1);
			big_data.push(rowData);
		});
				...
				if (resultat.success) {
					swal("Réussie", resultat.success, {
						...
						if (willDelete) {
							var listNumSTring =
								"listNum=" +
								encodeURIComponent(JSON.stringify(resultat.ListNum));
							var queryString =
