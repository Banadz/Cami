$(document).ready(function () {
	...
	$.ajax({
		...
		success: function (reponse, status) {
			...
			if (reponse.success) {
				...
				$("#idxnomprof").append(
					'<label class="labelprof" >' + reponse.user[0]["NOM_AG"] + "</label>"
				);
				$("#idxprenomprof").append(
					'<label class="labelprof" >' +
						reponse.user[0]["PRENOM_AG"] +
						"</label>"
				);
				$("#idxdivprof").append(
					'<label class="labelprof">' +
						reponse.user[0]["LABEL_DIVISION"] +
						"</label>"
				);
				$("#idxfonprof").after(
					'<label class="labelprof" id="labfonprof">' +
						reponse.user[0]["FONCTION_AG"] +
						"</label>"
				);
				if (reponse.user[0]["TYPE_AG"] != "USER") {
					if (reponse.user[0]["TYPE_AG"] == "Admin") {
						$("#labfonprof").after(
							' <span class="badge badge-pill badge-danger labelprof">Admin</span>'
						);
					} else {
						$("#labfonprof").after(
							' <span class="badge badge-pill badge-danger labelprof"><i class="ti-star">Admin</i></span>'
						);
					}
				}
	...
	$.ajax({
		...
		success: function (reponse, status) {
			...
			if (
				agentLogged["PASSWORD"] === "0000" ||
				agentLogged["PASSWORD"] === "default"
			) {
				swal({
					// id:"oldpassField",
