$(document).ready(function () {
	...
	var nomencltable = $(".nomencltable"),
		insertnomencl = $(".insertnomencl");
	...
	$(sortirbout2).each(function () {
		$(this).on("click", function (param) {
			...
			$("#lohahevitra1").remove();
			$("#headmatentres").append(
				'<h2 class="modal-title" id="lohahevitra1">Matériel : ' +
					matref1 +
					" - " +
					specmat1 +
					"</h2>"
			...
			$("#refsort1").append(
				'<input type="hidden" name="refmatentres" id="refmatentres" class="form-control" value="' +
					matref1 +
					'">'
			);
	...
	$(modifboutmat).each(function () {
		$(this).on("click", function (param) {
			...
			$("#title").remove();
			$("#header").append(
				'<h2 class="modal-title" id="title">Modification matériel : ' +
					refmat +
					"</h2>"
			);
	...
	$(editbout).each(function () {
		$(this).on("click", function (param) {
			...
			$("#title1").remove();
			$("#header1").append(
				'<h2 class="modal-title" id="title1">Modification matériel : ' +
					refmats +
					"</h2>"
			);
	...
	$(sortirbout).each(function () {
		$(this).on("click", function (param) {
			...
			$("#lohahevitra").remove();
			$("#head").append(
				'<h2 class="modal-title" id="lohahevitra">Matériel : ' +
					matref +
					" - " +
					specmat +
					"(" +
			...
			$("#refsort").append(
				'<input type="hidden" name="refmatsort" id="refmatsort" class="form-control" value="' +
					matref +
					'">'
			);
	...
	$(detbouton).each(function () {
		$(this).on("click", function (e) {
			...
			$("#titre").remove();
			$("#modalheader").append(
				'<h2 class="modal-title" id="titre">Matériel : ' +
					ref +
					" (" +
					spec +
					") </h2>"
				...
				$.ajax({
					...
					success: function (results, status) {
						...
						$.each(div, function (i, v) {
							...
							$("#rowdiv").append(
								'<div class="form-group col-3" id="division"><label for="" id="resultlab">Division</label><input type="text" style="text-align: center;" class="form-control" value="' +
									v["CODE_DIVISION"] +
									'" name="codediv" id="codediv"><input type="hidden" class="form-control" value="' +
									ref +
									'" name="referencemat" id="referencemat"></div>'
							);
	...
	$(changebout).each(function () {
		$(this).on("click", function (e) {
			...
			$("#title").remove();
			$("#modalhead").append(
				'<h2 class="modal-title" id="title">Matériel : ' +
					reference +
					" (" +
					specificite +
					") " +
					...
					$.ajax({
						...
						success: function (reponses, status) {
							...
							$.each(zay, function (i, v) {
								console.log(v["LABEL_DIVISION"]);
								$("#ambony").append(
									'<div class="form-group col-3" id="divrow"><label for="" class="resultlabel">Division</label><input type="text" id="codedivision" value="' +
										v["CODE_DIVISION"] +
	...
	$(infos).each(function () {
		$(this).on("click", function (eo) {
			...
			$("#matinfotitle").remove();
			$("#headmatinfo").append(
				'<h2 class="modal-title" id="matinfotitle">Matériel : ' +
					matrefinfo +
					" - " +
					specmatinfo +
					"</h2>"
								...
								success: function (reponse, status) {
									if (results.success) {
										swal({
											title: "Succès!",
											text:
												"Informations du matériel " + matrefinfo + " modifiée!",
											buttons: false,
											icon: "success",
										});
										setTimeout(function () {
